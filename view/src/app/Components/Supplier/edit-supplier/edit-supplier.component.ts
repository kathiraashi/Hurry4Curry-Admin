...
export class EditSupplierComponent implements OnInit {
  ...
  ngOnInit() {
    this.Form = new FormGroup({
      ...
      Supplier_Id: new FormControl(this.Supplier_Id, Validators.required),
  ...
  SetFormValues() {
    this.Form.controls['Supplier_Id'].setValue(this.Supplier_Id);
 ...
 Phone_AsyncValidate( control: AbstractControl ) {
  ...
  let Info = CryptoJS.AES.encrypt(JSON.stringify(Data), 'SecretKeyIn@123');
  Info = Info.toString();
  return this.Supplier_Service.Supplier_PhoneNumber_AsyncValidate({'Info': Info}).pipe(map( response => {
...
Submit() {
  if (this.Form.valid) {
     ...
     Info = Info.toString();
     this.Supplier_Service.Supplier_Update({ 'Info': Info }).subscribe( response => {
        const ResponseData = JSON.parse(response['_body']);
        if (response['status'] === 200 && ResponseData['Status'] ) {
           this.Toaster.NewToastrMessage({ Type: 'Success', Message: 'Supplier Details Successfully Updated' });
           this.router.navigate(['/Supplier_List']);
        } else if (response['status'] === 400 || response['status'] === 417 && !ResponseData['Status']) {
           this.Toaster.NewToastrMessage({ Type: 'Error', Message: ResponseData['Message'] });
        } else if (response['status'] === 401 && !ResponseData['Status']) {
           this.Toaster.NewToastrMessage({ Type: 'Error', Message: ResponseData['Message'] });
        } else {
           this.Toaster.NewToastrMessage({ Type: 'Error', Message: 'Updating Supplier Getting Error!, But not Identify!' });
        }
     });
  }
