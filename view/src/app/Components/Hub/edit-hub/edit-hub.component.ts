...
export class EditHubComponent implements OnInit {
   ...
   ngOnInit() {
      this.Form = new FormGroup({
         ...
         Hub_Id: new FormControl(this.Hub_Id, Validators.required),
   ...
   SetFormValues() {
      this.Form.controls['Hub_Id'].setValue(this.Hub_Id);
   ...
   UserName_AsyncValidate( control: AbstractControl ) {
      ...
      let Info = CryptoJS.AES.encrypt(JSON.stringify(Data), 'SecretKeyIn@123');
      Info = Info.toString();
      return this.Hub_Service.Hub_UserName_AsyncValidate({'Info': Info}).pipe(map( response => {
   ...
   Phone_AsyncValidate( control: AbstractControl ) {
      ...
      let Info = CryptoJS.AES.encrypt(JSON.stringify(Data), 'SecretKeyIn@123');
      Info = Info.toString();
      return this.Hub_Service.Hub_PhoneNumber_AsyncValidate({'Info': Info}).pipe(map( response => {
   ...
   Submit() {
      if (this.Form.valid) {
         ...
         Info = Info.toString();
         this.Hub_Service.Hub_Update({ 'Info': Info }).subscribe( response => {
            const ResponseData = JSON.parse(response['_body']);
            if (response['status'] === 200 && ResponseData['Status'] ) {
               this.Toaster.NewToastrMessage({ Type: 'Success', Message: 'Hub Details Successfully Updated' });
               ...
               this.Toaster.NewToastrMessage({ Type: 'Error', Message: 'Updating Hub Getting Error!, But not Identify!' });
            }
         });
      }
